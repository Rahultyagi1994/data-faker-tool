...
import {
  ...
  Download, Sparkles, CheckCircle2, ArrowDown, Menu, X,
} from 'lucide-react';
    ...
    <div className="min-h-screen" style={{ background: '#05050f', fontFamily: 'Inter, sans-serif' }}>
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 border-b border-white/5"
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between px-4 sm:px-6 h-14 sm:h-16">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <button onClick={onGetStarted}
                    className="px-3 sm:px-5 py-2 sm:py-2.5 rounded-lg sm:rounded-xl font-bold text-white text-xs sm:text-sm transition-all hover:scale-105"
        ...
        {mobileMenu && (
          <div className="md:hidden border-t border-white/5 px-4 py-4 space-y-3"
               style={{ background: 'rgba(5,5,15,0.95)', backdropFilter: 'blur(24px)' }}>
            <button onClick={() => navClick('#features')} className="block w-full text-left text-sm text-slate-400 hover:text-white font-medium py-2">Features</button>
            <button onClick={() => navClick('#healthcare')} className="block w-full text-left text-sm text-slate-400 hover:text-white font-medium py-2">Healthcare</button>
            <button onClick={() => navClick('#how-it-works')} className="block w-full text-left text-sm text-slate-400 hover:text-white font-medium py-2">How It Works</button>
      ...
      <section className="relative pt-24 sm:pt-32 pb-12 sm:pb-20 px-4 sm:px-6 overflow-hidden">
        <div className="absolute inset-0 pointer-events-none overflow-hidden">
          <div className="absolute w-[500px] sm:w-[900px] h-[500px] sm:h-[900px] rounded-full opacity-15 blur-[100px] sm:blur-[160px]"
               style={{ background: 'radial-gradient(circle, #f97316 0%, transparent 60%)', top: '-200px', left: '50%', transform: 'translateX(-50%)', animation: 'pulse1 8s ease-in-out infinite' }} />
          <div className="absolute w-[300px] sm:w-[600px] h-[300px] sm:h-[600px] rounded-full opacity-10 blur-[80px] sm:blur-[120px]"
               style={{ background: 'radial-gradient(circle, #6366f1 0%, transparent 60%)', bottom: '-100px', right: '-50px', animation: 'pulse2 10s ease-in-out infinite' }} />
        ...
        <div className={`relative z-10 max-w-5xl mx-auto text-center transition-all duration-1000 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          <div className="inline-flex items-center gap-2 px-3 sm:px-4 py-1.5 sm:py-2 rounded-full border mb-6 sm:mb-8"
               style={{ background: 'rgba(249,115,22,0.06)', borderColor: 'rgba(249,115,22,0.2)' }}>
            <div className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
            <span className="text-[10px] sm:text-xs font-bold text-slate-300">#1 Synthetic Data Generator</span>
            <span className="text-[9px] sm:text-[10px] font-bold text-orange-400 bg-orange-500/10 px-1.5 sm:px-2 py-0.5 rounded-full">FREE</span>
          </div>

          <h1 className="text-3xl sm:text-5xl md:text-6xl lg:text-8xl font-black text-white leading-[0.95] tracking-tight mb-4 sm:mb-6">
            Forge Realistic
            <br />
            ...
            <br />
            <span className="text-slate-400 text-2xl sm:text-4xl md:text-5xl lg:text-6xl">in Seconds</span>
          </h1>

          <p className="text-sm sm:text-lg md:text-xl text-slate-400 max-w-2xl mx-auto mb-8 sm:mb-10 leading-relaxed font-medium px-2">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-4 mb-10 sm:mb-14 px-4 sm:px-0">
            <button onClick={onGetStarted}
                    className="w-full sm:w-auto px-6 sm:px-8 py-3.5 sm:py-4 rounded-xl sm:rounded-2xl font-black text-white text-sm sm:text-base transition-all hover:scale-105 flex items-center justify-center gap-2.5"
            ...
            <a href="#features"
               className="w-full sm:w-auto px-6 sm:px-8 py-3.5 sm:py-4 rounded-xl sm:rounded-2xl font-bold text-slate-300 text-sm sm:text-base border border-white/15 hover:border-white/30 transition-all flex items-center justify-center gap-2"
          ...
          <div className="flex items-center justify-center gap-2 sm:gap-3 flex-wrap px-2">
            {[
              ...
              <span key={b.label} className="px-2.5 sm:px-3.5 py-1 sm:py-1.5 rounded-full text-[10px] sm:text-xs font-bold border flex items-center gap-1 sm:gap-1.5"
          ...
          <div className="mt-10 sm:mt-16 relative hidden sm:block">
                  ...
                  <table className="w-full text-xs min-w-[500px]">
                    <thead>
                      <tr className="border-b border-white/10">
                        ...
                        <th className="px-3 py-2 text-left text-emerald-400 font-bold"><span className="text-emerald-500/50 mr-1">◆</span>diagnosis</th>
                    ...
                    <tbody>
                      {[
                        { mrn: 'MRN-847293', name: 'James Wilson', diag: 'Type 2 Diabetes', med: 'Metformin 500mg' },
                        { mrn: 'MRN-512847', name: 'Charlotte Brown', diag: 'Essential Hypertension', med: 'Lisinopril 10mg' },
                        { mrn: 'MRN-938472', name: 'Oliver Garcia', diag: 'Acute Bronchitis', med: 'Azithromycin 250mg' },
                        { mrn: 'MRN-274859', name: 'Sophia Davis', diag: 'Major Depressive Disorder', med: 'Sertraline 50mg' },
                      ].map((r, i) => (
                        <tr key={i} className="border-b border-white/[0.04]">
                          ...
                          <td className="px-3 py-2.5 text-slate-400 font-mono">{r.diag}</td>
      ...
      <section className="py-8 sm:py-12 border-y border-white/5">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 text-center">
          <p className="text-[10px] sm:text-xs text-slate-700 font-bold uppercase tracking-[0.2em] mb-6 sm:mb-8">Trusted by developers & data teams worldwide</p>
          <div className="flex items-center justify-center gap-5 sm:gap-8 md:gap-14 flex-wrap opacity-30">
            {['Acme Corp', 'Nexus Tech', 'Zenith AI', 'Orbit Solutions', 'Stratus Cloud', 'Polaris Data'].map(c => (
              <span key={c} className="text-sm sm:text-lg md:text-xl font-black text-white/50 tracking-tight">{c}</span>
            ))}
          </div>
        </div>
      </section>
      ...
      {/* ── STATS SECTION ─────────────────────────────────────────── */}
      <section id="stats-section" className="py-12 sm:py-20 px-4 sm:px-6">
        <div className="max-w-6xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-6">
          {[
            ...
            <div key={s.label} className="rounded-xl sm:rounded-2xl p-4 sm:p-6 border border-white/10 text-center relative overflow-hidden group hover:border-white/20 transition-all"
              ...
              <div className="w-10 h-10 sm:w-14 sm:h-14 rounded-xl sm:rounded-2xl flex items-center justify-center mx-auto mb-3 sm:mb-4"
                   style={{ background: `${s.color}15`, border: `1px solid ${s.color}25` }}>
                <s.Icon className="w-5 h-5 sm:w-7 sm:h-7" style={{ color: s.color }} strokeWidth={1.8} />
              </div>
              <div className="text-2xl sm:text-4xl md:text-5xl font-black text-white mb-1">
                {s.value.toLocaleString()}{s.suffix}
              </div>
              <div className="text-[9px] sm:text-xs text-slate-500 font-bold uppercase tracking-wider">{s.label}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      <section id="features" className="py-12 sm:py-20 px-4 sm:px-6">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-10 sm:mb-16">
            <span className="text-[10px] sm:text-xs font-bold text-orange-400 uppercase tracking-[0.2em] mb-3 block">Features</span>
            ...
            <p className="text-slate-500 text-sm sm:text-lg max-w-2xl mx-auto px-2">From simple test data to complex healthcare records — Data Forge has you covered.</p>
          </div>
            ...
            {[
              {
                ...
                desc: 'Drag-and-drop fields, rename columns, change types — all in a live, reactive sidebar.'
              },
              {
                Icon: Zap, title: 'Instant Generation', color: '#fbbf24',
                desc: 'Generate up to 10,000 rows in milliseconds. No server calls — everything runs in your browser.'
              },
              {
                Icon: Heart, title: '28 Healthcare Fields', color: '#10b981',
                desc: 'Diagnoses, ICD codes, medications, vitals, lab results, nursing notes, insurance.'
              },
              {
                Icon: Settings, title: 'Custom Field Types', color: '#8b5cf6',
                desc: 'Build your own: Custom Lists, Patterns (regex-style), and Templates (compose with tokens).'
              },
              {
                Icon: Sparkles, title: '15 Ready-Made Scenarios', color: '#ec4899',
                desc: 'One-click presets for User Profiles, E-commerce, Analytics, HR, CRM, and 7 Healthcare scenarios.'
              },
              {
                Icon: Package, title: '4 Export Formats', color: '#06b6d4',
                desc: 'Download or copy as JSON, CSV, SQL INSERT statements, or XML. Optimized for any workflow.'
              },
              {
                Icon: Shield, title: 'HIPAA-Safe & Private', color: '#22c55e',
                desc: 'All data is generated client-side. Nothing is stored or uploaded. 100% synthetic.'
              },
              {
                Icon: Globe, title: 'Works Everywhere', color: '#3b82f6',
                desc: 'Works on desktop, tablet, and mobile. No installation — just open and forge.'
              },
              {
                Icon: Eye, title: 'Live Table Preview', color: '#f97316',
                desc: 'See your data instantly in table or code view. Copy or download with one click.'
              },
            ].map(f => (
              <div key={f.title} className="rounded-xl sm:rounded-2xl p-4 sm:p-6 border border-white/10 group hover:border-white/20 transition-all relative overflow-hidden"
                   style={{ background: 'rgba(255,255,255,0.02)' }}>
                ...
                <div className="w-10 h-10 sm:w-14 sm:h-14 rounded-xl sm:rounded-2xl flex items-center justify-center mb-3 sm:mb-4"
                     style={{ background: `${f.color}12`, border: `1px solid ${f.color}25` }}>
                  <f.Icon className="w-5 h-5 sm:w-7 sm:h-7" style={{ color: f.color }} strokeWidth={1.8} />
                </div>
      ...
      {/* ── HEALTHCARE SHOWCASE ────────────────────────────────────── */}
      <section id="healthcare" className="py-12 sm:py-20 px-4 sm:px-6 relative">
        <div className="absolute inset-0 pointer-events-none overflow-hidden">
          <div className="absolute w-[400px] sm:w-[600px] h-[400px] sm:h-[600px] rounded-full opacity-8 blur-[100px] sm:blur-[140px]"
               style={{ background: 'radial-gradient(circle, #10b981 0%, transparent 60%)', top: '50%', left: '50%', transform: 'translate(-50%,-50%)' }} />
        </div>

        <div className="relative max-w-6xl mx-auto">
          <div className="text-center mb-10 sm:mb-16">
            <span className="text-[10px] sm:text-xs font-bold text-emerald-400 uppercase tracking-[0.2em] mb-3 block">Healthcare Data</span>
            <h2 className="text-2xl sm:text-4xl md:text-5xl font-black text-white mb-3 sm:mb-4">
              The Most Complete
              <br />
            ...
            <p className="text-slate-500 text-sm sm:text-lg max-w-2xl mx-auto px-2">28 field types across 4 categories and 7 dedicated scenarios.</p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-6 sm:mb-10">
            {[
              ...
              },
              {
                ...
                fields: ['Hospital Ward', 'Room Number', 'Doctor Name', 'Admission', 'Discharge', 'Insurance', 'Copay'],
              },
            ].map(cat => (
              <div key={cat.label} className="rounded-xl sm:rounded-2xl p-4 sm:p-5 border transition-all group hover:border-white/20"
                   style={{ background: 'rgba(255,255,255,0.02)', borderColor: `${cat.color}20` }}>
                <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-lg sm:rounded-xl flex items-center justify-center mb-3"
                     style={{ background: `${cat.color}15`, border: `1px solid ${cat.color}25` }}>
                  <cat.Icon className="w-5 h-5 sm:w-6 sm:h-6" style={{ color: cat.color }} strokeWidth={1.8} />
                </div>
                <h4 className="text-xs sm:text-sm font-bold text-white mb-2 sm:mb-3">{cat.label}</h4>
                <div className="flex flex-wrap gap-1 sm:gap-1.5">
                  {cat.fields.map(f => (
                    <span key={f} className="px-1.5 sm:px-2 py-0.5 rounded-full text-[9px] sm:text-[10px] font-semibold border"
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2 sm:gap-3">
            {[
              ...
              <div key={sc.title} className="rounded-lg sm:rounded-xl p-3 sm:p-4 border border-emerald-500/10 hover:border-emerald-500/30 transition-all cursor-pointer group"
                   style={{ background: 'rgba(16,185,129,0.02)' }}
                   onClick={onGetStarted}>
                <div className="flex items-center gap-2 sm:gap-2.5 mb-1.5 sm:mb-2">
                  <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-lg flex items-center justify-center flex-shrink-0"
                       style={{ background: 'rgba(16,185,129,0.12)', border: '1px solid rgba(16,185,129,0.25)' }}>
                    <sc.Icon className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-emerald-400" strokeWidth={1.8} />
                  </div>
                  <span className="text-[10px] sm:text-xs font-bold text-emerald-200 group-hover:text-white transition-colors truncate">{sc.title}</span>
                </div>
                <div className="flex items-center gap-2 text-[9px] sm:text-[10px] text-emerald-600 font-mono">
      ...
      {/* ── HOW IT WORKS ──────────────────────────────────────────── */}
      <section id="how-it-works" className="py-12 sm:py-20 px-4 sm:px-6 border-t border-white/5">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-10 sm:mb-16">
            <span className="text-[10px] sm:text-xs font-bold text-orange-400 uppercase tracking-[0.2em] mb-3 block">How It Works</span>
            ...
            {[
              {
                ...
                desc: 'Pick from 50+ field types or create custom fields. Use a preset or build from scratch.',
                Icon: Layers,
              },
              {
                step: '02', title: 'Forge Your Data', color: '#fbbf24',
                desc: 'Hit the Forge button. Up to 10,000 rows generated instantly in your browser.',
                Icon: Zap,
              },
              {
                step: '03', title: 'Export & Ship', color: '#10b981',
                desc: 'Copy to clipboard or download as JSON, CSV, SQL, or XML.',
                Icon: ArrowDown,
              },
            ].map(s => (
              <div key={s.step} className="text-center relative">
                <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-xl sm:rounded-2xl flex items-center justify-center mx-auto mb-4 sm:mb-5 relative"
                     style={{ background: `${s.color}10`, border: `1px solid ${s.color}25` }}>
                  <s.Icon className="w-7 h-7 sm:w-9 sm:h-9" style={{ color: s.color }} strokeWidth={1.8} />
                  <span className="absolute -top-2 -right-2 w-6 h-6 sm:w-7 sm:h-7 rounded-full flex items-center justify-center text-[9px] sm:text-[10px] font-black text-white"
                ...
                <p className="text-xs sm:text-sm text-slate-500 leading-relaxed max-w-xs mx-auto">{s.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-16 sm:py-24 px-4 sm:px-6 relative overflow-hidden">
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute w-[500px] sm:w-[800px] h-[500px] sm:h-[800px] rounded-full opacity-10 blur-[120px] sm:blur-[160px]"
        ...
        <div className="relative max-w-3xl mx-auto text-center px-2">
          ...
          <p className="text-sm sm:text-lg text-slate-400 mb-8 sm:mb-10 max-w-xl mx-auto">
            Join thousands of developers who generate perfect synthetic data with Data Forge. Free forever.
          </p>
          <button onClick={onGetStarted}
                  className="px-8 sm:px-10 py-4 sm:py-5 rounded-xl sm:rounded-2xl font-black text-white text-base sm:text-lg transition-all hover:scale-105 flex items-center gap-2 sm:gap-3 mx-auto"
      ...
      <footer className="border-t border-white/5 py-8 sm:py-12 px-4 sm:px-6">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between pt-6 sm:pt-8 border-t border-white/5 gap-4">
            <p className="text-[10px] sm:text-xs text-slate-700">© {new Date().getFullYear()} Data Forge. All rights reserved.</p>
            <div className="flex items-center gap-4">
              {['Twitter', 'GitHub', 'Discord', 'LinkedIn'].map(s => (
                <a key={s} href="#" className="text-[10px] sm:text-xs text-slate-700 hover:text-white transition-colors font-medium">{s}</a>
              ))}
            </div>
          </div>
        </div>
